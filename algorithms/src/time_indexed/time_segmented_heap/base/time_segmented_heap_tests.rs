use crate::time_indexed::time_segmented_heap::TimeSegmentedHeap;
use chrono::{Duration, Utc};

#[test]
fn test_insert_and_expire_basic() {
    let heap = TimeSegmentedHeap::new(3, 10); // 3 buckets, each 10 seconds wide
...
fn test_out_of_range_insertion_ignored() {
    let heap = TimeSegmentedHeap::new(2, 10);
    let now = Utc::now().timestamp();
...
fn test_thread_safe_access() {
    ...
    let heap = TimeSegmentedHeap::new(3, 10);
    let now = Utc::now().timestamp();
    let h1 = heap.clone();
    let h2 = heap.clone();
...
fn test_large_number_of_inserts() {
    let heap = TimeSegmentedHeap::new(10, 5);
    let now = Utc::now().timestamp();
...
fn test_concurrent_expiry_does_not_panic() {
    ...
    let heap = Arc::new(TimeSegmentedHeap::new(5, 10));
    let now = Utc::now().timestamp();
