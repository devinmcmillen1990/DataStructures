...
#[derive(Debug, Clone)]
pub struct TimeSegmentedHeap<T> {
    inner: std::sync::Arc<std::sync::Mutex<TimeSegmentedHeapInner<T>>>,
}

#[derive(Debug)]
struct TimeSegmentedHeapInner<T> {
    ...
    T: Ord + Clone + Hash + Eq + Send + 'static,
{
    pub fn new(num_buckets: usize, resolution_secs: i64) -> Self {
        ...
        Self {
            inner: std::sync::Arc::new(std::sync::Mutex::new(TimeSegmentedHeapInner {
                buckets,
